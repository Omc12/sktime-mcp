...
results.append(
    check_value("np.array float", np.array([1.1, 2.2, 3.3]), [float(x) for x in [1.1, 2.2, 3.3]])
)
...
results.append(
    check("np.float64 nan", np.float64("nan"))
)  # json.dumps maps NaN to null via str workaround
